...
// 所有奖品的全量缓存
// 从Redis奖品池获取id奖品数量
func GetGiftPoolNum(id int) int64 {
	key := "gift_pool"
	rds := dataSource.RedisInstCache()
	rs, err := rds.Do("HGET", key, id)
	if err != nil {
		log.Printf("prize_data.GetGiftPoolNum redis hget key[%v] error[%v]", key, err)
		return 0
	}
	return comm.GetInt64(rs, 0)
}
...
func prizeServGift(id int) bool {
	...
	// 默认值 -1
	num := comm.GetInt64(rs, -1) //这里给了一个默认值是-1
	if num >= 0 {
		return true
	} else {
		return false
...
func ImportCacheCodes(id int, code string) bool {
	...
	key := fmt.Sprintf("gift_code_%v", id)
	rds := dataSource.RedisInstCache()
	_, err := rds.Do("SADD", key, code)
	if err != nil {
		log.Printf("prize_data.ImportCacheCodes redis sadd key[%v] error[%v]", key, err)
		return false
	}
	return true
}
...
func RecacheCodes(giftId int, codeService services.CodeService) (succNum, failNum int) {
	...
	//list := codeService.Search(giftId) 这个接口没写
	list := codeService.GetAll()
...
func prizeServCodeDiff(giftId int, codeService services.CodeService) string {
	...
	// 优惠券发放成功,更新数据库中发放状态
	codeService.UpdateByCode(&models.Code{
		Code:       code,
		SysStatus:  2,
...
// 奖品的发奖计划
func ResetGiftPrizeData(giftInfo *models.Gift, giftService services.GiftService) {
		...
		for _, gift := range list {
			...
			if err != nil {
				...
				if index > 0 {
					...
					err = giftService.Update(&models.Gift{
						Id:        gift.Id,
						PrizeData: string(str),
					}, columns)
					if err != nil {
